...
| **你的注意力** | 重要消息实时桌面弹窗，延迟 < 1 秒 | Windows Toast 通知，watchdog 文件监听 |
| **你的偏好** | 持续学习什么对你重要，什么是噪音 | 每日对账 + 隐式行为信号，贝叶斯权重更新 |
...
| 每天 21:00 | **通知质量对账**——汇总今日推送，收集反馈，更新重要性权重 |
...
大多数通知系统永远发同样的提醒，不管你在不在乎。Aegis 会学习。

**评分管线** — 每条新消息在进入 AI 之前先被打分：
```
分数 = 联系人有用率 × 关键词倍率 × 时间模式 × 连续消息数
```
...
| 信号 | 触发时机 | 权重 |
|---|---|---|
| 焦点事项被完成 ✅ | 你标记了某条任务为已完成 | 强正向 |
| 焦点事项被删除 ❌ | 你直接删掉了它 | 强负向 |
| 你快速回复了对方 📱 | 收到通知后 30 分钟内回复 | 正向 |
| 24 小时无后续行动 ⏳ | 通知被忽略 | 弱负向 |
| 显式反馈 ✍️ | 你回复 `Aegis: 对账 有用 1,3 噪音 2` | 最强信号 |

**每天 21:00 对账报告** — Aegis 发给你一份总结：
```
📊 今日通知质量报告 — 4月10日
共推送 8 条：✅ 有用 5  ❓ 待确认 3  🔇 噪音 0

以下通知没有检测到后续行动，请告诉我是否有用：
  1. [14:32] 张三: "合同请在明天前确认签字..."
  2. [16:45] 工作群: "本周周报模板已上传..."
  3. [20:10] 李四: "明天上午你有空吗..."

回复：Aegis: 对账 有用 1,3  噪音 2
```

权重用贝叶斯滚动均值更新——早期反馈影响大，样本多了之后趋于保守。
存在 `data/learned_importance.json`，纯 JSON，可以直接查看和手动调整。
